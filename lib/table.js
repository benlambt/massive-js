...
Table.config = function(opts) {
  DocumentTable.setGenid(opts.genid);
};

//a simple alias for returning a single record
Table.prototype.findOneRow = function(args, next){
  if(_.isFunction(args)){
    next = args;
    args = {};
  }
  this.findRow(args, function(err,results){
    if(err){
      next(err,null);
    }else{
...
Table.prototype.findRow = function(){
...
Table.prototype.insertRow = function(data, next) {
...
Table.prototype.updateRow = function(fields, where, next){
...
Table.prototype.saveRow = function(args, next){
  ...
  if(this.containsPk(args)){
    ...
    where[pk] = args[pk];
    delete args[pk];
    this.updateRow(args,where,next);
  }else{
    this.insertRow(args,next);
  }
};

Table.prototype.remove = function(args, next){
  if (_.isString(args)) {
    var id = args;
    args = {};
    args[this.primaryKeyName()] = id;
  }
...
Table.prototype.searchRow = function(args, next){
